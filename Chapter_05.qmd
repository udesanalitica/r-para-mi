...
: ANOVA para un diseño en bloques completos al azar {#tbl-anova}
...
Debido a esta variabilidad, resulta apropiado emplear un Diseño en Bloques Completos al Azar (DBCA), ya que permite controlar parcialmente el efecto de factores ambientales no controlados mediante la agrupación de unidades experimentales homogéneas en bloques. En este estudio se evaluó la respuesta del cultivo de frijol (Phaseolus vulgaris) a diferentes tratamientos de fertilización que combinan fuentes químicas y biológicas, incluyendo bioinoculantes y abonos orgánicos. El objetivo fue determinar si existen diferencias significativas en el crecimiento del cultivo y generar evidencia que contribuya al desarrollo de estrategias de manejo agronómico más sostenibles. La Figura @fig-DBCA_frijol presenta las plantas utilizadas en el experimento.

![Plantas de *Phaseolus vulgaris* utilizadas en el experimento de fertilización bajo un Diseño en Bloques Completos al Azar (DBCA).](images/4_DBCA/DBCA_frijol.png){#fig-DBCA_frijol width="60%"}

**Para mayor información puede consultar:** Christian Andrés Chacín Zambrano, Laboratorio de Cultivos Vegetales, Universidad de Santander (UDES), 2025. Experimento de fertilización en Phaseolus vulgaris bajo diseño en bloques completos al azar. Reproducidos con autorización del autor.\*.
...
La variable respuesta, denominada `Altura_cm`, corresponde a una variable cuantitativa continua asociada al crecimiento vegetal, expresada en valores decimales.
...
| **Altura_cm** | Cuantitativa continua | Variable respuesta asociada al crecimiento de las plantas, expresada en valores decimales (por ejemplo, altura de planta en cm). |

: Descripción de las variables de la base de datos {#tbl-dbca}
...
DBCA <- read_delim(
  "data/DBCA_Frijol/DBCA_frijol.csv",
  delim = ";",
...
```{R}
file.exists("data/DBCA_Frijol/DBCA_frijol.csv")
```

::: callout-note
Para que el lector lo descargue sin clonar el repositorio\
library(readr)\
url_datos \<- "https://raw.githubusercontent.com/udesanalitica/r-para-mi/main/data/DBCA_Frijol/DBCA_frijol.csv"\
DBCA \<- read_csv(url_datos)
:::

[Descargar los datos (CSV)](data/DBCA_Frijol/DBCA_frijol.csv){download="DBCA_frijol.csv"}
...
La variable `Altura_cm`, por su parte, se mantiene como una variable cuantitativa continua.

```{r}
DBCA$Tratamiento <- as.factor(DBCA$Tratamiento)
DBCA$Bloque      <- as.factor(DBCA$Bloque)
DBCA$Altura_cm   <- as.numeric(DBCA$Altura_cm)
```
...
```{r}
modeloDBCA <- aov(Altura_cm ~ Tratamiento + Bloque,
                  data = DBCA)

summary(modeloDBCA)
```

**Interpretación:** el análisis del modelo lineal evidenció que el factor `tratamiento` presentó un efecto altamente significativo sobre la variable respuesta ($F = 153.749$; $p < 2 \times 10^{-16}$), indicando que existen diferencias estadísticas entre las medias de los tratamientos evaluados.
...
La normalidad de los residuos del modelo se evaluó mediante la prueba de Shapiro-Wilk, la cual contrasta la hipótesis nula de que los residuos provienen de una distribución normal. Valores de $𝑝> 0.05$ indican que no existen evidencias suficientes para rechazar dicho supuesto.
...
**Interpretación:** el gráfico Q-Q mostró que los residuos estandarizados se distribuyen cercanos a la línea diagonal de referencia, sin evidenciar desviaciones importantes o patrones sistemáticos. Esto sugiere que los residuos presentan una distribución aproximadamente normal, respaldando visualmente el cumplimiento del supuesto de normalidad requerido para el análisis de varianza (ANOVA).
...
```{r}
bartlett.test(Altura_cm ~ Tratamiento, data = DBCA)
car::leveneTest(Altura_cm ~ Tratamiento, data = DBCA)
fligner.test(Altura_cm ~ Tratamiento, data = DBCA)
```

**Interpretación:** las pruebas de Bartlett, Levene y Fligner-Killeen presentaron valores de ppp superiores al nivel de significancia de 0.05, indicando que no existen evidencias estadísticas suficientes para rechazar la hipótesis nula de igualdad de varianzas entre tratamientos.
...
El valor de DW varía entre 0 y 4, valores próximos a 2 indican independencia, y las desviaciones hacia 0 o hacia 4 señalan autocorrelación negativa o positiva, respectivamente.

```{r}
dwtest(modeloDBCA)
```

**Interpretación:** la prueba de Durbin-Watson arrojó un valor observado de $DW = 2.042$ y un valor de $p = 0.4159$, superior al nivel de significancia de 0.05. En consecuencia, no existen evidencias estadísticas suficientes para rechazar la hipótesis nula de independencia de los residuos.

Adicionalmente, la cercanía del estadístico $DW$ al valor teórico de 2; 2.042 respalda la ausencia de autocorrelación entre los errores experimentales. Por lo tanto, se considera cumplido el supuesto de independencia requerido para la validez del análisis de varianza bajo el Diseño en Bloques Completos al Azar (DBCA).

Adicionalmente, el estadístico $DW = 2.042$ se encuentra muy próximo al valor de referencia de 2, que corresponde a la ausencia de autocorrelación, lo cual respalda el cumplimiento del supuesto de independencia.
...
**Interpretación:** la prueba de Tukey HSD aplicada al Diseño en Bloques Completos al Azar (DBCA) evidenció diferencias estadísticamente significativas entre todos los tratamientos evaluados sobre la altura de las plantas de frijol. El Tratamiento 4 presentó la mayor media (64.78 cm; grupo a), seguido del Tratamiento 3 (59.71 cm; grupo b), el Tratamiento 2 (54.26 cm; grupo c) y el Tratamiento 1 (50.29 cm; grupo d), el cual registró la menor media.
...
**Interpretación:** la prueba LSD (*Least Significant Difference*) aplicada al Diseño en Bloques Completos al Azar (DBCA) evidenció diferencias estadísticamente significativas entre las medias de altura de las plantas de frijol para todos los tratamientos evaluados. El Tratamiento 4 presentó la mayor altura promedio (64.78 cm; grupo a), seguido del Tratamiento 3 (59.71 cm; grupo b), el Tratamiento 2 (54.26 cm; grupo c) y finalmente el Tratamiento 1 (50.29 cm; grupo d), el cual registró la menor media.
...
```{r}
ggplot(DBCA, aes(x = Tratamiento, y = Altura_cm, fill = Tratamiento)) + geom_boxplot() + theme_minimal() + labs( title = "Distribución del resultado por tratamiento", x = "Tratamiento", y = "Altura_m")
```

**Interpretación:** la gráfica de cajas y bigotes muestra la distribución de las alturas de las plantas de *Phaseolus vulgaris* bajo los diferentes tratamientos de fertilización evaluados mediante un Diseño en Bloques Completos al Azar (DBCA). Se observa una tendencia creciente en las medianas de los tratamientos, evidenciando un efecto progresivamente favorable sobre el crecimiento vegetal desde NPK Comercial hasta Orgánico + NPK.
