...
### Cargar e instalar automáticamente los paquetes necesarios para el análisis
...
paquetes <- c("readxl", "nlme", "ggplot2", "emmeans", "dplyr", "multcomp", "multcompView")
...
# Convertir 'Tiempo' a factor/discreto para un mejor manejo del 'dodge'
plot_data$Tiempo_Factor <- factor(plot_data$Tiempo)

ggplot(plot_data, aes(x = Tiempo_Factor, y = emmean,
                      color = Tratamiento, group = Tratamiento)) +
  # 1. Aplicar posición de 'esquive' a las líneas y puntos
  geom_line(aes(y = emmean), position = position_dodge(0.4), size = 1.2) +
  geom_point(position = position_dodge(0.4), size = 3) +

  # 2. Aplicar posición de 'esquive' a las barras de error
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL),
                width = 0.2, position = position_dodge(0.4)) +

  # 3. Aplicar posición de 'esquive' a las etiquetas de texto (las letras)
  # Asegúrate de que el 'dodge' sea consistente (0.4)
  geom_text(aes(label = .group),
            vjust = -0.8, # Mueve la etiqueta ligeramente por encima de la barra
